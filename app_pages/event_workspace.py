...
def show_event_workspace(eid, get_data, db):
    ...
    # Handling potential lowercase column from Supabase
    ...
    if h2.button("⬅️ Back to Home", use_container_width=True):
        st.session_state.page = "🏠 Event Hub"; st.rerun()
    ...
    # ==========================================
    # 📊 TAB 1: OVERVIEW (Saves to 'Events' & 'Event_Contacts')
    # ==========================================
    with tab_ov:
        ...
        edit_mode = col_ov_ed.toggle("🔓 Edit Details", value=False) if is_adm else False
        ...
        with st.form("edit_core_details", border=True):
            ...
            with c1:
                ...
                new_multi = st.selectbox("Multi-Day Event?", ["Yes", "No"], index=0 if is_multi else 1, disabled=not edit_mode)
        ...
        current_contacts = df_con[df_con[id_col] == eid] if not df_con.empty else pd.DataFrame()
            ...
            for idx, row in current_contacts.iterrows():
                with st.expander(f"{row['Role']}: {row['Name']}"):
                    ...
                    if is_adm and st.button("🗑️ Remove", key=f"del_{idx}"):
                        # CHANGE 2: db.update_table
                        db.update_table("Event_Contacts", df_con.drop(idx))
                        st.rerun()
        ...
        with st.popover("➕ Add New Contact", use_container_width=True):
            with st.form("add_contact_form", clear_on_submit=True):
                ...
                if st.form_submit_button("💾 Save Contact", use_container_width=True):
                    ...
                    # CHANGE 3: db.update_table
                    db.update_table("Event_Contacts", pd.concat([df_con, pd.DataFrame([new_c])], ignore_index=True))
                    st.success(f"Added {c_name}!"); st.rerun()

    # ==========================================
    # 🚛 TAB 2: LOGISTICS (Saves to 'Logistics_Details')
    # ==========================================
    with tab_log:
        st.subheader("🚛 Logistics & Setup")
        df_log = get_data("Logistics_Details")
        log_match = df_log[df_log[id_col] == eid] if not df_log.empty else pd.DataFrame()
        curr_log = log_match.iloc[0] if not log_match.empty else {}

        edit_log = st.toggle("🔓 Edit Logistics", value=False) if is_adm else False

        with st.form("logistics_form"):
            new_bump_in = st.text_input("Bump In", value=str(curr_log.get("Bump_In", "08:00")), disabled=not edit_log)
            new_bump_out = st.text_input("Bump Out", value=str(curr_log.get("Bump_Out", "18:00")), disabled=not edit_log)
            new_setup = st.selectbox("Setup Type", ["Marquee", "Food Truck", "Indoor", "Cart"], disabled=not edit_log)
            if st.form_submit_button("💾 Save Logistics", disabled=not edit_log):
                new_log_data = {"Event_ID": eid, "Setup_Type": new_setup, "Bump_In": new_bump_in, "Bump_Out": new_bump_out}
                df_log = pd.concat([df_log[df_log[id_col] != eid], pd.DataFrame([new_log_data])], ignore_index=True)
                # CHANGE 4: db.update_table
                db.update_table("Logistics_Details", df_log)
                st.success("Logistics Updated!"); st.rerun()

    # ==========================================
    # 📝 TAB 3: DAILY REPORT (Saves to 'Event_Reports')
    # ==========================================
    with tab_rep:
        rep_date_str = selected_report_date.strftime('%Y-%m-%d')
        st.subheader(f"📝 Report: {selected_report_date.strftime('%A, %d %b')}")
        df_rep = get_data("Event_Reports")
        day_match = df_rep[(df_rep[id_col] == eid) & (df_rep['Report_Date'] == rep_date_str)] if not df_rep.empty else pd.DataFrame()
        curr_rep = day_match.iloc[0] if not day_match.empty else {}

        with st.form("daily_rep"):
            weather = st.selectbox("☀️ Weather", ["Sunny", "Cloudy", "Rainy", "Windy"], index=0)
            gen = st.text_area("✍️ General Comments", value=str(curr_rep.get("General_Comments", "")))
            if st.form_submit_button("💾 Save Daily Report"):
                new_r = {"Event_ID": eid, "Report_Date": rep_date_str, "Weather": weather, "General_Comments": gen}
                mask = (df_rep[id_col] == eid) & (df_rep['Report_Date'] == rep_date_str)
                df_rep = pd.concat([df_rep[~mask], pd.DataFrame([new_r])], ignore_index=True)
                # CHANGE 5: db.update_table
                db.update_table("Event_Reports", df_rep)
                st.success("Report Saved!"); st.rerun()

    # ==========================================
    # 👥 TAB 4: STAFFING (Saves to 'Event_Staffing')
    # ==========================================
    with tab_staff:
        ...
        assigned_names = df_staffing[df_staffing[id_col] == eid]['Staff_Name'].tolist() if not df_staffing.empty else []

        # Logic for assigning/removing staff...
        # [Simplified for brevity - ensure you use db.update_table("Event_Staffing", df_staffing)]
        st.write("Current Assigned:", assigned_names)

    # ==========================================
    # 💰 TAB 5: SALES (Saves to 'Event_Sales')
    # ==========================================
    with tab_sales:
        st.subheader("💰 Sales Entry")
        df_sales = get_data("Event_Sales")
        
        with st.form("sales_form"):
            v_eftpos = st.number_input("Card / Eftpos", min_value=0.0)
            v_cash = st.number_input("Cash", min_value=0.0)
            if st.form_submit_button("💾 Save Sales Record"):
                new_sales = {
                    "Event_ID": eid, "Event_Date": selected_report_date.strftime('%Y-%m-%d'),
                    "Total_Gross_Sales": v_eftpos + v_cash, "Eftpos": v_eftpos, "Cash": v_cash
                }
                df_sales = pd.concat([df_sales, pd.DataFrame([new_sales])], ignore_index=True)
                # CHANGE 6: db.update_table
                db.update_table("Event_Sales", df_sales)
                st.success("Sales Recorded!"); st.rerun()
# import streamlit as st
# import pandas as pd
# import re
# from datetime import datetime
