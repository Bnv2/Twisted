...
def migrate_ui():
    ...
    if st.button("Finalize Event_Sales Migration"):
        ...
        try:
            ...
            # 3. EXACT MAPPING to your SQL Schema
            # We map Google Sheet headers (Left) to your DB columns (Right)
            rename_map = {
                "Cash": "cash_sales",        # Matches your SQL 'cash_sales'
                "Eftpos": "card_sales",      # Matches your SQL 'card_sales'
                "Card": "card_sales",        # Fallback
                "Opening Float": "opening_float",
                "Closing Float": "closing_float",
                "Total": "total_revenue"
            }
            
            # Apply renaming and force everything to lowercase/stripped
            df_cleaned = df_cleaned.rename(columns=rename_map)
            df_cleaned.columns = [c.lower().strip() for c in df_cleaned.columns]

            # 4. Filter for ONLY columns in your SQL table
            # 'id' is generated by DB, so we usually don't send it unless it's in the Sheet
            db_columns = ['event_id', 'opening_float', 'cash_sales', 'card_sales', 'closing_float', 'total_revenue']
            
            # Keep only the columns that exist in both the Sheet and the DB
            final_cols = [c for c in db_columns if c in df_cleaned.columns]
            df_final = df_cleaned[final_cols]

            # 5. Push to Supabase
            data_dict = df_final.to_dict(orient='records')
            
            # Use insert because 'id' is auto-generated (UUID) 
            # and you likely don't have a unique constraint on event_id here
            db.client.table("event_sales").insert(data_dict).execute()
            
            st.success(f"✅ Event_Sales: Migrated {len(df_final)} rows!")
            st.balloons()
